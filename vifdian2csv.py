...
import numpy as np
import re
infile = open("./file/222.vif", 'r', encoding='utf-8')
allPos = [] #保存名称对应的坐标
lastAns = [] #保存原始名称
lastname = [] #保存原始名称
point = []
...
for i in infile:
    ...
    xy = np.zeros((1, 2), dtype=np.int32)
    if axis_id == 1:
        x = np.array(x)
        y = np.array(y)
        axis_id = 0
...
for name in  lastname:
    point.append([name,cx[ii],cy[ii]])
    ii += 1
data2 = pd.DataFrame(point, index=None, columns=["name","center_point_x","center_point_y"])
#data.to_csv("./file/info.csv", index=None)
data2.to_csv("./file/ff.csv", index=None)
